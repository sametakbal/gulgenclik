<template>
  <div>
    <div v-if="authenticated" class="home row">
      <div class="col-md-3">
        <div class="card text-end text-bg-light mt-5" v-for="billiard in americans" v-bind:key="billiard.id">
          ...
          <div class="card-img-overlay">
            ...
            <p class="card-text">
              {{billiard.using ? `${billiard.hour} saat ${billiard.minute} dk` : ''}}
              <br>
              {{billiard.using ? `Toplam tutar : ${billiard.totalSessionCost} ₺` : ''}}
            </p>
            <button v-if="!billiard.using" @click="openSession(billiard.id)" type="button" class="btn btn-primary">
              Oturumu
              Başlat
            </button>
            <button v-else type="button" @click="closeSession(billiard.id)" class="btn btn-danger">Oturumu Kapat
            </button>
          </div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="card text-end text-bg-light mt-5" v-for="billiard in threeBalls" v-bind:key="billiard.id">
      ...
      <div class="col-md-6">
        <div class="justify-content-start" v-if="selectedSession.id">
          ...
          <ul class="list-group" id="ordered-buffet-contents" v-if="buffetContents">
            <li v-for="(content,index) in buffetContents" v-bind:key="index"
                class="list-group-item d-flex justify-content-between align-items-center">
              {{ index + 1 }}. {{ content.name }}
              <button @click="addBuffetContent(content)" class="badge bg-secondary text-white"> Ekle </button>
            </li>
          </ul>
          ...
          <ul class="list-group" id="ordered-buffet-contents" v-if="selectedSession.buffetContents">
            <li v-for="(content,index) in selectedSession.buffetContents" v-bind:key="index"
                class="list-group-item d-flex justify-content-between align-items-center">
              {{ index + 1 }}. {{ content.name }}
